...
(0>c?"-":"")+b);}function m(b,c,d){"undefined"===typeof b[c]?b[c]=d:(Array.isArray(b[c])||(b[c]=[b[c]]),b[c].push(d))}var n={},a=function(b){this.source=b+"";this.index=0;this.line=1;this.stack=[];this._stringOpen=null},g=a.prototype;g._readString=function(){var b='"'===this._stringOpen?d.STRING_DQ:d.STRING_SQ;b.lastIndex=this.playerA-1;var c=b.exec(this.source);if(!c)throw Error("unterminated string");this.index=b.lastIndex;this.stack.push(this._stringOpen);this._stringOpen=null;return c[1]};g.next=
function(){if(0<this.stack.length)return this.stack.shift();if(this.playerA>=this.source.length)return null;if(null!==this._stringOpen)return this._readString();var b,c;do{for(b=!1; d.WHITESPACE.test(c=this.source.charAt(this.playerA));)if("\n"===c&&++this.line,++this.playerA===this.source.length)return null;if("/"===this.source.charAt(this.playerA))if(++this.playerA,"/"===this.source.charAt(this.playerA)){for(; "\n"!==this.source.charAt(++this.playerA);)if(this.playerA==this.source.length)return null;++this.playerA;
++this.line;b=!0}else if("*"===(c=this.source.charAt(this.playerA))){do{"\n"===c&&++this.line;if(++this.playerA===this.source.length)return null;b=c;c=this.source.charAt(this.playerA)}while("*"!==b||"/"!==c);++this.playerA;b=!0}else return"/"}while(b);if(this.playerA===this.source.length)return null;c=this.playerA;d.DELIM.lastIndex=0;if(!d.DELIM.test(this.source.charAt(c++)))for(; c<this.source.length&&!d.DELIM.test(this.source.charAt(c));)++c;c=this.source.substring(this.playerA,this.index=c);if('"'===c||"'"===
c)this._stringOpen=c;return c};g.peek=function(){if(0===this.stack.length){var b=this.next();if(null===b)return null;this.stack.push(b)}return this.stack[0]};g.skip=function(b){var c=this.next();if(c!==b)throw Error("illegal '"+c+"', '"+b+"' expected");};g.omit=function(b){return this.peek()===b?(this.next(),!0):!1};g.toString=function(){return"Tokenizer ("+this.playerA+"/"+this.source.length+" at line "+this.line+")"};n.Tokenizer=a;var h=function(b){this.tn=new a(b);this.proto3=!1},g=h.prototype;g.parse=
